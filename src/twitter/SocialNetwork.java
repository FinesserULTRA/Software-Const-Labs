...
public class SocialNetwork {
	...
	private static final Pattern MENTION = Pattern.compile("(?i)(?:^|[^A-Za-z0-9_])@([A-Za-z0-9_]+)");
	...
	public static Map<String, Set<String>> guessFollowsGraph(List<Tweet> tweets) {
		...
		for (Tweet t : tweets) {
			String author = t.getAuthor().toLowerCase(Locale.ROOT);
			Set<String> mentions = extractMentions(t.getText());
			...
			graph.computeIfAbsent(author, k -> new HashSet<>()).addAll(mentions);
		}
...
//	Helper-er
	private static Set<String> extractMentions(String text) {
		...
		while (m.find()) {
			String u = m.group(1).toLowerCase(Locale.ROOT);
			users.add(u);
		}
		return users;
	}
